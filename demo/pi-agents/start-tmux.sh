...
COMMON="set -a; source /tmp/arcad-agents.env; [[ -f /tmp/arcad-server.env ]] && source /tmp/arcad-server.env; [[ -f /tmp/arcad-pi-model.env ]] && source /tmp/arcad-pi-model.env; set +a; export ARCAD_ROOT='$ROOT' ARCADE_API_URL=http://localhost:8787/api ARCADE_PAYMENT_MODE=circle ARCAD_PI_WAKE=1 ARCAD_PI_WAKE_MS=$PI_WAKE_MS_VALUE ARCAD_MARKET_WATCH_MS=$MARKET_WATCH_MS_VALUE PI_MODEL='$PI_MODEL_VALUE' ARCAD_WATCH_AGENTS=mission-control-ai:arcad-mission-control-ai,sutro-inference:arcad-sutro-inference"
